...
!
!-----------------------------------------------------------------------
! QE-CONVERSE override of QE 7.5 PW/src/new_nsg.f90.
! Both entry points are kept together to avoid duplicate archive symbols.
SUBROUTINE new_nsg()
  !-----------------------------------------------------------------------
  ...
  !
  ! CONVERSE uses the full k mesh without antiunitary symmetries.
  IF (gamma_only) CALL errore('new_nsg', 'Complex occupations require explicit k points', 1)
  IF (nsym /= 1 .OR. t_rev(1) /= 0) &
     CALL errore('new_nsg', 'Complex CONVERSE occupations require identity symmetry only', 1)
  !
